...
function renderSummary(data, wrapper, addressId) { 
  var output = "";
  for (var key in data) {
    if (key !== "id")  output += `<span> <strong> ${key.replace("EcomOrderCustomer", "")}: </strong> ${data[key]} </span><br>`;
  }
...
item.addEventListener('keyup', e => {
  ...
  for (var key in currentObj) {
    var id = key.replace('Customer', 'Delivery');
    if ((document.getElementById(key) !== null)) currentObj[key] = document.getElementById(id).value;          
  }

  renderSummary(currentObj, 'summary-shipping', addressId);
  if(editBilling.checked) {
    renderSummary(currentObj, 'summary-billing', addressId);
  }
})
})
...
item.addEventListener('keyup', e => {
  ...
  var currentObj = addresses.filter(o => o.id == addressId)[0];
  for (var key in currentObj) {
    if ((document.getElementById(key) !== null)) currentObj[key] = document.getElementById(key).value;          
  }
  renderSummary(currentObj, 'summary-billing', addressId)    
});
});
